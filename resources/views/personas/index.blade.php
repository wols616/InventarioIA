        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex justify-between items-center">
                ...
                @if(isset($authUser) && ($authUser->persona->rol->nombre ?? '') === 'Admin')
                    <a href="{{ route('personas.create') }}" class="bg-brand-600 hover:bg-brand-700 text-white font-medium py-2 px-4 rounded-md transition duration-200 flex items-center">
                ...
                @elseif(isset($authUser) && ($authUser->persona->rol->nombre ?? '') === 'Auditor')
                    @include('partials.disabled-button', ['label' => 'Crear Persona'])
                @endif
            </div>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($personas as $p)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">#{{ $p->id_persona }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <div class="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center">
                                            <span class="text-indigo-600 font-medium text-sm">{{ substr($p->nombre, 0, 1) }}{{ substr($p->apellido ?? '', 0, 1) }}</span>
                                        </div>
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">{{ $p->nombre }} {{ $p->apellido }}</div>
                                        <div class="text-sm text-gray-500">{{ $p->dui ?? 'Sin DUI' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($p->correo)
                                    <div class="text-sm text-gray-900">{{ $p->correo }}</div>
                                @else
                                    <span class="text-sm text-gray-400">Sin correo</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $p->rol ? $p->rol->nombre : 'Sin rol' }}</div>
                                <div class="text-sm text-gray-500">{{ $p->departamento ? $p->departamento->nombre : 'Sin departamento' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if(isset($p->estado))
                                    @if($p->estado)
                                    ...
                                    @endif
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">Sin estado</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('personas.show', $p) }}" class="text-brand-600 hover:text-brand-900 p-2 hover:bg-brand-50 rounded transition duration-200" title="Ver">
                                    ...
                                    @if(isset($authUser) && ($authUser->persona->rol->nombre ?? '') === 'Admin')
                                        <a href="{{ route('personas.edit', $p) }}" class="text-gray-600 hover:text-gray-900 p-2 hover:bg-gray-50 rounded transition duration-200" title="Editar">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">

...
@section('content')
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-gray-200">
            <h1 class="text-2xl font-bold text-gray-900">Crear Movimiento</h1>
        </div>
            ...
            <form action="{{ route('movimientos.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" id="activo_search" placeholder="Buscar activo por código, marca o modelo..." class="w-full mb-2 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent">
                        <select name="id_activo" id="activo_select" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent">
                            ...
                            @foreach($activos as $a)
                                <option value="{{ $a->id_activo }}" data-ubicacion="{{ $a->id_ubicacion_actual }}" {{ (old('id_activo', $presetActivo ?? '') == $a->id_activo) ? 'selected' : '' }}>{{ $a->codigo }} - {{ $a->marca }} {{ $a->modelo }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="id_ubicacion_origen" id="origen_select" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent">
                            ...
                            @foreach($ubicaciones as $u)
                                <option value="{{ $u->id_ubicacion }}">{{ $u->nombre }} ({{ $u->codigo_interno ?? '' }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="id_ubicacion_destino" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent">
                            ...
                            @foreach($ubicaciones as $u)
                                <option value="{{ $u->id_ubicacion }}" {{ old('id_ubicacion_destino') == $u->id_ubicacion ? 'selected' : '' }}>{{ $u->nombre }} ({{ $u->codigo_interno ?? '' }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="fecha_movimiento" id="fecha_movimiento" value="{{ old('fecha_movimiento', date('Y-m-d')) }}" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent">
                    </div>

                    <div class="md:col-span-2">
                        <label for="motivo" class="block text-sm font-medium text-gray-700 mb-2">Motivo</label>
                        <textarea name="motivo" id="motivo" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent">{{ old('motivo') }}</textarea>
                    </div>
                </div>
    ...
    <script>
        (function(){
            ...
            // Filtrado simple de opciones del select usando el input de búsqueda
            if(activoSearch){
                activoSearch.addEventListener('input', function(){
                    const q = this.value.trim().toLowerCase();
                    Array.from(activoSel.options).forEach(opt => {
